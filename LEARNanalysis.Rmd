...
nlights = "16"  # 4, 8, 16, 27
nlightstuple = "2"  # 2 or 3
hier = "flat"  # hierarchical or flat
...
data_dir = paste(base_dir, "/nlights_", nlights, "/nlightstuple_", nlightstuple, "/", hier, "/", learning_signal, "/alpha_", alpha, "/lambda_", lambda, sep = "")
  ...
  for (tr in 0:max(event_hist$trial)) {
    ...
    for (op in unique(theta_history$option)) {
      ...
      if (nrow(theta_dat) > 0) {
        ...
        gg_theta = gg_theta +
          aes(action, feature) +
          scale_fill_gradient(low = "white", high = "red", limits = c(min(theta_history$value), max(theta_history$value)))  # necessary to make each plot based on the same color
...
if (gg_save) {
  ...
  ggsave(file.path(plot_dir, "gg_final_policies.png"), gg_final_policies, width = 10, height = 5)
}
```
```{r}
```
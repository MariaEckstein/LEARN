...
n_actions = "8" #16   # can be 8 (8 basic actions, 3 levels), can be 16 (16 basic actions, 4 levels), or 27 (27 basic actions, 3 levels)
...
for (tr in unique(option_history$trial)) {
  plot_dat = subset(option_history, trial == tr)
  gg_option_history = ggplot(plot_dat, aes(action, level)) +
    ...
    facet_wrap(~ step)
  ggsave(file.path(plot_dir, paste(n_actions, "gg_option_history", tr, ".png", sep = "")), gg_option_history, width = 3 * length(unique(plot_dat$step)), height = 2)
}
...
for (op in unique(theta_history$option)) {
  for (tr in unique(theta_history$trial)) {
    plot_dat = subset(theta_history, option == op & trial == tr)
    gg_theta_history = ggplot(plot_dat, aes(action, feature)) +
      geom_tile(aes(fill = value), colour = "white") +
    scale_fill_gradient(low = "black", high = "white",
                        breaks = c(0, 0.5, 1), labels = c(-1, 0, 1), limits = c(-1, 1)) +  # necessary to make each plot based on the same color
      labs(title = paste("trial", tr))
    filename = file.path(plot_dir, paste(n_actions, "gg_theta_history_option", op, "trial", tr, ".png", sep = ""))
    ggsave(filename, gg_theta_history_option, width = 2, height = 2)
  }
}
```
...
for (tr in 0:max(event_hist$trial)) {
  ...
  ggsave(file.path(plot_dir, paste(n_actions, "grob_event_value_", tr, ".png", sep = "")), grob_event_value,

...
data_dir = paste(base_dir, "/n_basic_actions_", n_basic_actions, "/", hier, "/", learning_signal, "/alpha_", alpha, "/lambda_", lambda, sep = "")
...
gg_features_over_time = ggplot(th0, aes(trial, value, color = factor(action))) +
  ...
  labs(color = "Action") +
  facet_grid(~ factor(feature, labels = paste("Feature", unique(th0$feature))))

gg_actions_over_time = ggplot(th0, aes(trial, value, color = factor(feature))) +
  labs(color = "Feature") +
  geom_point(position = "jitter", alpha = .3) +
  facet_grid(~ factor(action, labels = paste("Feature", unique(th0$action))))

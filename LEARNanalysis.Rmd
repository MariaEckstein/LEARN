...
data_dir = paste(base_dir,
                 "/n_options_per_level_", n_options_per_level,
...
# Event history
event_hist = read.csv(file = file.path(data_dir, "event_history_long_e0_a0.csv"), header = T)
event_hist$X = NULL
head(event_hist)
summary(event_hist)

# Novelty history
n_history = read.csv(file = file.path(data_dir, "n_history_long_e0_a0.csv"), header = T)
n_history$X = NULL
n_history$n = n_history$value
n_history$value = exp(-as.numeric(n_lambda) * n_history$value)
head(n_history)
summary(n_history)

# Value history
v_history = read.csv(file = file.path(data_dir, "v_history_long_e0_a0.csv"), header = T)
v_history$X = NULL
if (hier == "flat") {
...
# State history
state_history = read.csv(file = file.path(data_dir, "state_history_long_e0_a0.csv"), header = T)
state_history$X = NULL
head(state_history)
summary(state_history)

# Option history
option_history = read.csv(file = file.path(data_dir, "option_history_long_e0_a0.csv"), header = T)
option_history$X = NULL
head(option_history)
summary(option_history)

# Theta history
theta_history = read.csv(file = file.path(data_dir, "theta_history_long_e0_a0.csv"), header = T)
...
# Elig. trace history
# e_history = read.csv(file = file.path(data_dir, "e_history_long_e0_a0.csv"), header = T)
# e_history$X = NULL
# head(e_history)
# summary(e_history)
```
...
for (tr in 0:max(event_hist$trial)) {
  ...
  gg_novel = gg_event
  gg_novel$data = subset(n_history, trial == tr)
  gg_novel = gg_novel +
    scale_fill_gradient(low = "white", high = "red", limits = c(0, upper_limit))  # necessary to make each plot based on the same color
  ...
  #   gg_e = gg_value
  #   gg_e$data = subset(e_history, trial == (tr+1))
  #   gg_e = gg_e +
  #     aes(action, feature) +
  ...
  for (op in unique(theta_history$option)) {
    # if (hier == "hierarchical") {
    #   gg_e$data = subset(e_history, option == op & trial == (tr+1))
    # }
    ...
    if (nrow(theta_dat) > 0) {
      ...
      if (gg_save) {
        ggsave(file.path(file_path, paste("trial", tr, ".png", sep = "")), grob_theta_option, width=max(event_hist$action), height=max(event_hist$level))
      }
    }
  }
  ...
  if (gg_save) {
    ggsave(file.path(file_path, paste("trial", tr, ".png", sep = "")), grob_event_value, width=max(event_hist$action), height=max(event_hist$level))
  }
}

...
# Theta history
theta_history = read.csv(file = file.path(data_dir, "theta_history_long.csv"), header = T, na.strings = c("", "NA", "-1"))
...
for (tr in 0:max(event_hist$trial)) {
  ...
  ggsave(file.path(plot_dir, paste("grob_event_value_", tr, ".png", sep = "")), grob_event_value,
...
ggplot(v_history, aes(trial, value, color = factor(action), group = level)) +

...
if (plot_all_trials) {
  for (tr in 2:max(event_hist$trial)) {
    ...
    if (gg_save) {
      ggsave(file.path(file_path, paste("trial", tr, ".png", sep = "")), grob_event_value, width=max(event_hist$action), height=3*max(event_hist$level))
    }
  }
}

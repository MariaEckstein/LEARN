...
class HierarchicalAgent(object):
    ...
    def __init__(self, alpha, epsilon, n_levels, n_lights, n_lights_tuple):
        ...
        self.o_v = np.zeros([2, n_lights, self.n_options - n_lights + 1])  # COULD I INITIALIZE THE WHOLE THING AS NAN? inter-option policies; each option has one slice of dimension [1, n_lights] or [2, n_lights], corresponding to the values of the actions/options it can select
    ...
    def take_action(self, state):
        ...
        if True:  # len(action) == 1 or self.level == 0:  # agent selected a basic action
            selected_a = action[-1]
    ...
    def get_values(self, state):
        ...
        if len(self.o_list) == 0:  # if option stack is empty, i.e., we have the choice among all actions and options
            ...
            available_v = self.o_v[:, :, self.o_list[-1]]  # get the values of the last option in the stack
    ...
    def handle_options(self, high_lev_change, action):
        high_level_changes = np.argwhere(high_lev_change)
        for i in range(len(high_level_changes)):  # if more than one higher-level lights turned on during one move
            level, light_i = high_level_changes[i]
        ...
        # break out of option at each time step with prob gamma

    def create_option(self, change_position, option_index):
        self.o_blocked[change_position[0], change_position[1]] = False  # Un-block the option
        self.v[change_position[0] + 2, change_position[1]] = 1  # initialize option value at 1
        self.o_v[:, :, option_index] = np.nan  # block out all actions available to the option

...
n_lights = 81   # number of level-0 lights (formerly know as "blue" lights); must be n_lights_tuple ** x
n_lights_tuple = 3  # number of lights per level-0 tuple
n_agents = 50
alpha = 0.75  # agent's learning rate
epsilon = 0.05  # inverse of agent's greediness
n_trials = 2500  # number of trials in the game
...
v_off = np.zeros([n_trials, n_agents])
v_on = np.zeros([n_trials, n_agents])
for ag in range(n_agents):
    ...
    for trial in range(n_trials):
        ...
        high_lev_change = environment.high_lev_change
        agent.update_values(old_state, action, new_state, high_lev_change)
        lights_on[trial, ag] = np.sum(new_state[0])
        v_on[trial, ag] = np.average(agent.v[1])
        v_off[trial, ag] = np.average(agent.v[0])

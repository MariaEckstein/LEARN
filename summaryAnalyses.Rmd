...
```{r Analyze my HN agent in detail}
# Read in data
...
gg_summary_novelty = ggplot(subset(algo_n), aes(trial, value, group = factor(action), color = factor(action), fill = factor(action))) +#, env_id == 4
...
if (gg_save) {
  ggsave(file.path(sum_plot_dir, "gg_summary_value.png"), gg_summary_value, width = 10, height = 2)
  ggsave(file.path(sum_plot_dir, "gg_summary_novelty.png"), gg_summary_novelty, width = 10, height = 2)
}
...
if (gg_save) {
  ggsave(file.path(sum_plot_dir, "gg_theta.png"), gg_theta, width = 3, height = 5)
}
